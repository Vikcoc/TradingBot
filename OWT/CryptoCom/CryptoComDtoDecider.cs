...
namespace OWT.CryptoCom
{
    public class CryptoComDtoDecider
    {
        ...
        public void AddHandler(ICryptoComDtoExecutor executor) => _cryptoComDtoExecutors.Add(executor);

        public async Task<int> Execute(JObject? dto, CryptoComMarketClient marketClient, CancellationToken cancellationToken)
        {
            if (dto == null)
                return 0;
            var cnt = 0;
